...
class AvgReadout(nn.Module):
    ...
    def forward(self, emb, mask=None):                    # mask是邻接矩阵
        vsum = torch.mm(mask, emb)                        # 聚合周围邻居的embedding
        row_sum = torch.sum(mask, 1)                      # 计算邻居数量
        row_sum = row_sum.expand((vsum.shape[1], row_sum.shape[0])).T          
        global_emb = vsum / row_sum                       # 计算邻居的平均embedding
          
        return F.normalize(global_emb, p=2, dim=1)        # 按行(dim=1)进行L2(p=2)Normalization
...
class Encoder(Module):
    ...
    def reset_parameters(self):
        torch.nn.init.xavier_uniform_(self.weight1)      #使用xavier初始化，可以使得每层的方差一样，进而使得输出值落在激活函数的有效区间内
    ...
    def forward(self, feat, feat_a, adj):
        ...
        g = self.read(emb, self.graph_neigh)    # Normalized邻居的embedding
        g = self.sigm(g)  

        g_a = self.read(emb_a, self.graph_neigh)
        g_a = self.sigm(g_a)  

        ret = self.disc(g, emb, emb_a)          # 真假节点embedding和真邻居embedding
        ret_a = self.disc(g_a, emb_a, emb)      # 真假节点embedding和假邻居embedding
...
class Encoder_map(torch.nn.Module):               # 随机梯度下降优化映射函数
